(function () {
    ...
    // --- [FUNÃ‡Ã•ES DE MANIPULAÃ‡ÃƒO DE ÃUDIO] ----------------------------
    ...
    function play(audio, newSource = null) {
        ...
        // Adiciona evento 'canplay' para garantir que o Ã¡udio pode ser reproduzido
    ...
    async function getDataFrom({ artist, title, art, cover }) {
        ...
        try {
            ...
            // 2. Em caso de erro geral, tenta o iTunes como Ãºltimo recurso
            dataFrom = await getDataFromITunes(artist, title, art, cover);
        }
    ...
    async function getDataFromSearch(artist, title, defaultArt, defaultCover) {
        ...
        try {
            ...
            if (!response.ok) {
                throw new Error(`Erro na requisiÃ§Ã£o para o novo endpoint de busca. Status: ${response.status}`);
            }
    ...
    async function getDataFromITunes(artist, title, defaultArt, defaultCover) {
        ...
        try {
            ...
            if (!response.ok) {
                throw new Error(`Erro na requisiÃ§Ã£o para o iTunes. Status: ${response.status}`);
            }
            const data = await response.json();
    ...
    function normalizeTitle(api) {
        ...
        // Verifica se a API retorna informaÃ§Ãµes no formato "last_played"
        if (api.last_played) {
          title = api.last_played.song;
          artist = api.last_played.artist;
        // Verifica se a API retorna informaÃ§Ãµes diretamente em "song" e "artist"
    ...
    // --- [FUNÃ‡Ã•ES DE MANIPULAÃ‡ÃƒO DA INTERFACE] ------------------------
    ...
    // --- [ATUALIZA ELEMENTOS DA PÃGINA DA ESTAÃ‡ÃƒO] ------------------
    ...
    // --- [FUNÃ‡Ã•ES DE ATUALIZAÃ‡ÃƒO DE CONTEÃšDO] ----------------------- 
    ...
    function setHistory(data, current, server) {
        ...
        const promises = data.map(async (item) => {
            ...
            // Verifica se Ã© Deezer e se stream_url Ã© invÃ¡lida
            if (server === 'deezer' && !dataFrom.stream_url) { 
                dataFrom.stream_url = '#'; // Define como '#' para evitar link invÃ¡lido
            }
    ...
    function initApp() {
        ...
        // Define o evento de clique para o botÃ£o play/pause
        if (playButton !== null) {
        ...
        function init(current) {
            ...
            // Se a url da estaÃ§Ã£o atual for diferente da estaÃ§Ã£o atual, atualiza a informaÃ§Ã£o
            ...
            fetch(jsonUri)
                ...
                .then(async (res) => {
                    ...
                    // Se currentSong for diferente da mÃºsica atual, atualiza a informaÃ§Ã£o
                    const title = current.title;
                    if (currentSongPlaying !== title) {
                        // Atualizar a mÃºsica atual
                        ...
                        // Verificar se o tÃ­tulo e o artista nÃ£o sÃ£o undefined
                        if (title && artist) {
                            const dataFrom = await getDataFrom({
                                artist,
                                title,
                            ...
                            // Estabelecer dados da mÃºsica atual
                            ...
                            console.log("TÃ­tulo ou artista undefined, nÃ£o serÃ¡ feita a busca pela capa do Ã¡lbum.");
                        }
                    }
                })
    ...
    const historyTemplate = `<div class="history-item flex items-center g-1">
        ...
        window.addEventListener("DOMContentLoaded", () => {
            ...
            // Adiciona o event listener para iniciar o audio apÃ³s primeiro click na tela
